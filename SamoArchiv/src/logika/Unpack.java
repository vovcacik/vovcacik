...
class Unpack {
	...
	/**
	 * Do složky cilDir rozbalí všechny dané záznamy (entries) z JAR archivu
	 * jarFile.
	...
	static void unpack(File cilDir, ZipFile jarFile, java.util.List<ZipZaznam> entries) {
		...
		try {
			...
			for (ZipZaznam entry : entries) {
				...
				FileOutputStream fos = new FileOutputStream(cilDir.getAbsolutePath() + entry.getName().substring(Archiv.TRUNK.length()));
